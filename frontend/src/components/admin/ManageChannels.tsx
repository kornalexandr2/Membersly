    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            <form onSubmit={submit} className="bg-neutral-900 p-8 rounded-[2.5rem] border border-white/10 space-y-4 shadow-xl">
                <h3 className="text-sm font-black uppercase text-blue-500 tracking-[0.2em] italic">{editing ? 'Update Resource' : 'Link New Resource'}</h3>
                <div className="flex flex-wrap gap-4">
                    <input disabled={!!editing} placeholder="TELEGRAM CHAT ID" className="bg-black p-4 rounded-2xl flex-1 min-w-[200px] text-xs font-bold border border-white/5 outline-none focus:border-blue-500" value={form.chat_id || ''} onChange={e => setForm({...form, chat_id: e.target.value})} />
                    <input placeholder="TITLE" className="bg-black p-4 rounded-2xl flex-1 min-w-[200px] text-xs font-bold border border-white/5 outline-none focus:border-blue-500" value={form.title || ''} onChange={e => setForm({...form, title: e.target.value})} />
                    {!editing && (
                ...
                <div className="space-y-2">
                    <textarea 
                        ...
                        placeholder="Welcome Message (Use {name} for user's name)..." 
                    ...
                    <div className="flex items-center gap-3 ml-2">
                        ...
                        <span className="text-[10px] font-black text-neutral-500 uppercase tracking-widest">Pin Welcome Message</span>
                    </div>
                </div>
                <button className="w-full bg-blue-600 py-4 rounded-2xl font-black text-[10px] uppercase shadow-lg hover:bg-blue-500">{editing ? 'Save Changes' : 'Link Matrix'}</button>
            </form>
                ...
                {data.map((c: any) => (
                    <div key={c.id} className="p-6 bg-neutral-900 rounded-[2rem] border border-white/5 flex justify-between items-center group">
                        ...
                        <div className="flex items-center gap-4">
                            <button onClick={() => { setEditing(c.id); setForm(c); }} className="text-white/40 font-black text-[10px] uppercase hover:text-white transition">Edit</button>
