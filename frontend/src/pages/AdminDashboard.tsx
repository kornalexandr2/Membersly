        ...
        <div className="p-4 max-w-6xl mx-auto space-y-16 animate-in fade-in duration-700">
            <div className="flex flex-col items-center gap-10 text-center">
                ...
                <div className="w-full overflow-x-auto no-scrollbar pb-4">
                    <div className="flex bg-neutral-900 p-2.5 rounded-[2.5rem] border border-white/10 shadow-2xl w-max mx-auto">
                        {['stats', 'channels', 'tariffs', 'broadcast', 'users', 'coupons', 'bots', 'settings'].map((v) => (
                            <button key={v} onClick={() => setView(v)} className={`px-6 md:px-10 py-3 md:py-4 rounded-[2rem] text-[10px] md:text-xs font-black uppercase transition-all duration-300 whitespace-nowrap ${view === v ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/40 scale-105 z-10' : 'text-neutral-300 hover:text-white hover:bg-white/5'}`}>{t(`admin.${v}`)}</button>
                        ))}
                    </div>
                </div>
            </div>
