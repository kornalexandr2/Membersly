...
export const AdminDashboard = ({ token, apiUrl }: { token: string, apiUrl: string }) => {
    ...
    return (
        <div className="p-4 max-w-6xl mx-auto space-y-16 animate-in fade-in duration-700">
            <div className="flex flex-col items-center gap-10 text-center">
                <div className="text-6xl font-black uppercase italic tracking-tighter text-white drop-shadow-[0_0_30px_rgba(59,130,246,0.2)]">MEMBERSLY</div>
                <div className="w-full flex justify-center">
                    <div className="flex bg-neutral-900 p-2.5 rounded-[2.5rem] border border-white/10 overflow-x-auto no-scrollbar shadow-2xl max-w-full">
                        {['stats', 'channels', 'tariffs', 'broadcast', 'users', 'coupons', 'bots', 'settings'].map((v) => (
                            <button key={v} onClick={() => setView(v)} className={`px-10 py-4 rounded-[2rem] text-xs font-black uppercase transition-all duration-300 whitespace-nowrap ${view === v ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/40 scale-110 z-10' : 'text-neutral-300 hover:text-white hover:bg-white/5'}`}>{t(`admin.${v}`)}</button>
                        ))}
                    </div>
                </div>
            </div>
